/*
  # Minimal Student Name Migration

  Run each section separately if needed to diagnose issues.
*/
...
-- STEP 5: Simple RLS policies (no complex joins)
...
CREATE OR REPLACE FUNCTION use_invite_link(p_invite_code TEXT, p_joining_user_id UUID)
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM org_members WHERE org_id = v_link.org_id AND user_id = p_joining_user_id) THEN
  ...
  VALUES (v_link.org_id, p_joining_user_id, 'student', v_link.id, 'active');
