  ...
  if (useAdminNav && adminItems) {
    return (
      <div className={`py-2 ${className}`}>
        ...
        {adminItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center gap-2.5 mx-3 px-3 py-1 rounded-xl transition-all duration-300 ${
                isActive
        ...
        <div className="px-6 pt-2 mt-2 border-t border-gray-200/50">
          <Link
            to="/foundations"
    ...
    <div className={`py-0.5 ${className}`}>
      {studentNavGroups.map((group) => (
        <div key={group.heading || '_top'}>
          {group.heading && (
            <div className="px-6 pt-1.5 pb-0">
      ...
      {(effectiveRoleInfo.role === 'super_admin' || effectiveRoleInfo.role === 'org_admin') && (
        <div className="px-5 pt-2 mt-1.5 border-t border-gray-200/50">
          <Link
      ...
      {roleInfo.role === 'super_admin' && (
        <div className="px-3 pt-2 mt-1.5 border-t border-gray-200/50">
          <div className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5 px-1">
            View As
          </div>
